...
const followAndUnfollow = async (req, res, next) => {
  ...
  try {
    ...
    if (
      ...
      throw new BadRequestError('profile deesnt exist!');
    }
...
const FriendList = async (req, res, next) => {
  try {
    ...
    if (!profile)
      throw new ForbiddenError('you have to create a profile first');
    if (profile.friends.length <= 0) res.status(200).json({ friends: [] });
    res.status(200).json({
      success: true,
...
const friendRequests = async (req, res, next) => {
  ...
  if (!profile) throw new ForbiddenError('you have to create a profile first!');
...
const acceptFriendRequest = async (req, res, next) => {
  ...
  try {
    ...
    if (!requestSenderProfile) throw new NotFoundError('user not found!');
    await Profile.updateOne(
      { user: req.user._id },
      {
        $pull: {
