...
try {
  ...
  const profiles = await Profile.find({user:{$in:allPostAuthors}}).populate('posts')
  
  for(let profile of profiles){
    const getPosts = profile.posts.filter(post=>post.privicy.toUpperCase() !== "PRIVATE")
    friendsAndFollowingPosts.concat(getPosts)
  }
